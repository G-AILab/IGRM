...
def get_scores(edges_pos, edges_neg, A_pred, adj_label, device):
    ...
    preds_all = adj_rec.view(-1).long().to(device)
...
def train_model(adj_train, adj_label, val_edges, val_edges_false, test_edges, test_edges_false, features, vgae, device, epochs = 200, lr = 0.01, criterion = 'roc'):
    optimizer = torch.optim.Adam(vgae.parameters(), lr=lr)
    ...
    for epoch in range(epochs):
        ...
        optimizer.zero_grad()
        loss = log_lik = norm_w*F.binary_cross_entropy_with_logits(A_pred, adj_label)
        ...
        r = get_scores(val_edges, val_edges_false, A_pred, adj_label,device)
